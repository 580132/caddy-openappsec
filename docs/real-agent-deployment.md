...
| Attack traffic passes, no block, no agent logs | The module sent only REQUEST_START and never REQUEST_END, so the engine's terminal verdict (end_request) never fired; or the agent policy is in a detect/learn mode | Use the fixed image (`>= 01d55b4`) which streams the full transaction (START→HEADER→BODY→END); verify the agent policy is `prevent`, not `detect-learn` (local_policy.yaml / cloud console) |
